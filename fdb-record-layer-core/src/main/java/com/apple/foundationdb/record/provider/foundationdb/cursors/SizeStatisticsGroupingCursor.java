...
public class SizeStatisticsGroupingCursor implements RecordCursor<SizeStatisticsGroupedResults> {
    ...
    public CompletableFuture<RecordCursorResult<SizeStatisticsGroupedResults>> onNext() {
        ...
        return subspaceProvider.getSubspaceAsync(context).thenCompose(subspace -> {
            KeyValueCursor kvCursor = KeyValueCursor.Builder
                    ...
                    .build();
            return AsyncUtil.whileTrue(() -> kvCursor
                            .onNext().thenApply(nextKv -> {
                                if (nextKv.hasNext()) {
                                    ...
                                    if (nextKv.getNoNextReason() == NoNextReason.SOURCE_EXHAUSTED) {
                                        // Send the last result with a continuation that will then turn into a END
                                        nextStatsResult = RecordCursorResult.withNextValue(new SizeStatisticsGroupedResults(currentGroupingKey, intermediateResults), new SizeStatisticsGroupingContinuation());
                                    } else {
                                        ...
                                        nextStatsResult = RecordCursorResult.withoutNextValue(new SizeStatisticsGroupingContinuation(nextKv, intermediateResults, currentGroupingKey), nextKv.getNoNextReason());
                                    }
                                    return false;
                                }
                            }))
    ...
    private boolean groupBreak(final Tuple currentGroupingKey, final Tuple nextGrouingKey) {
        return (!Objects.equals(currentGroupingKey, nextGrouingKey));
    }

    // form a continuation that allows us to restart statistics aggregation from where we left off
    private static class SizeStatisticsGroupingContinuation implements RecordCursorContinuation {
        @Nullable
         ...
         */
        public SizeStatisticsGroupingContinuation() {
        }

        /**
         * The inner cursor still has results, we can continue (e.g. group break).
         */
        private SizeStatisticsGroupingContinuation(@Nonnull RecordCursorResult<KeyValue> currentKvResult,
                                                   @Nonnull SizeStatisticsResults sizeStatisticsResults,
        ...
        public ByteString toByteString() {
            if (cachedByteString == null) {
                ...
                if (sizeStatisticsResults != null) {
                    builder.setPartialResults(sizeStatisticsResults.toProto());
                }
     ...
     */
    @Nonnull
    public static SizeStatisticsGroupingCursor ofRecords(@Nonnull FDBRecordStore store, @Nonnull FDBRecordContext context,
     ...
     */
    @Nonnull
    public static SizeStatisticsGroupingCursor ofIndex(@Nonnull FDBRecordStore store, @Nonnull String indexName, @Nonnull FDBRecordContext context,
                                                       @Nonnull ScanProperties scanProperties, @Nullable byte[] continuation, int aggregationDepth) {
     ...
     */
    @Nonnull
    public static SizeStatisticsGroupingCursor ofIndex(@Nonnull FDBRecordStore store, @Nonnull Index index, @Nonnull FDBRecordContext context,
                                                       @Nonnull ScanProperties scanProperties, @Nullable byte[] continuation, int aggregationDepth) {
